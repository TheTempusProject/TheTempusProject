...
use TempusProjectCore\DatabaseModel;
...
class Message extends DatabaseModel
{
    ...
    public function getInbox($limit = null)
    {
        ...
        $limit = [0, $limit];
        $messageData = self::$db->get('messages', ['parent', '=', 0, "AND", 'userFrom', '=', App::$activeUser->ID, "OR", 'parent', '=', 0, "AND", 'userTo', '=', App::$activeUser->ID], 'ID', 'DESC', $limit);
        if ($messageData->count() == 0) {
    ...
    public function getOutbox($limit = null)
    {
        ...
        $limit = [0, $limit];
        $messageData = self::$db->get('messages', ['parent', '=', 0, "AND", 'userFrom', '=', App::$activeUser->ID], 'ID', 'DESC', $limit);
        if ($messageData->count() == 0) {
            Debug::info('No messages found');
            return false;
        }
    ...
    private function processMessage($messageArray, $filters = [])
    {
        ...
        foreach ($messageArray as $message) {
            ...
            if ($last->isRead == 0) {
                $messageOut['unreadBadge'] = Views::standardView('message.unreadBadge');
            } else {
    ...
    public function getUnreadCount( $userId ) {
        $result = self::$db->get('messages', ['userTo', '=', $userId, "AND", 'isRead', '=', 0, "AND", 'parent', '=', 0, "AND", 'recieverDeleted', '=', 0]);
        return $result->count();
    }
    ...
    public function markRead($id)
    {
        ...
        $result = self::$db->get('messages', ['ID', '=', $id, 'AND', 'userTo', '=', App::$activeUser->ID, 'AND', 'isRead', '=', '0']);
        if ($result->count() == 0) {
